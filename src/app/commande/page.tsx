...
import {commandePropsType, detailType, variationType} from "@/types";
import {useFetchShoesById} from "@/hook/useFetchShoesById";
...
export default function Commande() {
    const commandeId = useSelector((state: commandePropsType) => state.commande.id);
                        ...
                        <div className="wrapper flex flex-col gap-20">
                            <div className="flex lg:flex-nowrap flex-wrap gap-10">
                                <div className="lg:w-4/12 w-full flex flex-col gap-4 grow">
                                    ...
                                    <Link href={commandeId ? `./sneakers/${oneShoes.slug}` : `./sneakers`}
                                          className="text-slate-500 text-sm order-first flex items-center gap-2 hover:text-blue-500 transition"><span><FiArrowLeft/></span> {commandeId ? oneShoes?.title : "Sneakers"}
                                    </Link>
                                    <div
                                    ...
                                    {/* Product selected */}
                                    <div
                                        className={`lg:min-h-80 flex flex-col gap-4 ${!commandeId ? "h-64 items-center justify-center " : "items-start justify-start h-auto"}`}>
                                        {!commandeId ?
                                    ...
                                    <div className="grid grid-cols-2 gap-4 w-full">
                                        ...
                                        <div
                                            ...
                                            <p className={`${montserrat.className} text-md font-medium text-slate-700`}>Sous
                                                Total : </p>
                                            <span className="block">{commandeId ? `${oneShoes.price} €` : "_"}</span>
                                        </div>
                                        ...
                                        <div
                                            ...
                                            <span
                                                className="block font-bold">{commandeId ? `${oneShoes.price} €` : "_"}</span>
                                        </div>
